...
class AnalyzeContext {
	...
	void initCursor() {
		...
		this.segmentBuff[this.cursor] = CharacterUtil
				.regularize(this.segmentBuff[this.cursor], cfg.isEnableLowercase());
		...
		for (; index <= this.cursor;) {
			...
			if (path != null) {
				...
				while (l != null) {
					...
					/*
					 * int innerIndex = index + 1; for (; innerIndex < index +
					 * l.getLength(); innerIndex++) { Lexeme innerL =
					 * path.peekFirst(); if (innerL != null && innerIndex ==
					 * innerL.getBegin()) { this.outputSingleCJK(innerIndex -
					 * 1); } }
					 */
